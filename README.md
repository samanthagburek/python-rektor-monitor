...
```bash
    python3 main.py --inclusion <logIndex of signed artifact> --artifact <artifact file>
  ```
...
```bash
    python3 main.py --consistency --tree-id <treeid in latest checkpoint> --tree-size <treesize in latest checkpoint> --root-hash <roothash in latest checkpoint>
  ```
